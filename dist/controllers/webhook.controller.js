...
exports.receiveWhatsappChat = exports.verifyWebhook = void 0;
...
const manualChatSchema = zod_1.z.object({
...
const verifyWebhook = (req, res) => {
    try {
        ...
        if (mode && token) {
            if (mode === 'subscribe' && token === env_1.env.META_WA_VERIFY_TOKEN) {
...
const verifyMetaSignature = (req) => {
    ...
    if (!env_1.env.META_APP_SECRET || env_1.env.META_APP_SECRET.trim() === '') {
 ...
 */
const receiveWhatsappChat = async (req, res, next) => {
    try {
        ...
        if (env_1.env.META_APP_SECRET && env_1.env.META_APP_SECRET.trim() !== '' && !verifyMetaSignature(req)) {
        ...
        if (body.object === 'whatsapp_business_account') {
            ...
            for (const msg of messages) {
                ...
                await (0, whatsapp_queue_1.addChatToQueue)(senderPhone, messageText);
            }
        ...
        if (manualResult.success) {
            ...
            const job = await (0, whatsapp_queue_1.addChatToQueue)(sender, message);
            res.status(200).json({
                status: 'queued',
...
exports.receiveWhatsappChat = receiveWhatsappChat;
